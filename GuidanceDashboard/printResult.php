...
if (isset($_REQUEST["facultyId"])) {
    ...
    $sql = "SELECT questionformtable.questionType,AVG(resulttable.rates) AS RESULT FROM resulttable INNER JOIN facultyformtable ON facultyformtable.facultyId = resulttable.facultyId INNER JOIN questionformtable ON questionformtable.questionID = resulttable.questionID WHERE facultyformtable.facultyId='$facultyId' AND resulttable.resultStatus = 'ACTIVE' GROUP BY questionformtable.questionType; ";
    $result = $db->query($sql);
